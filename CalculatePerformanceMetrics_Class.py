...
class CalculatePerformanceMetrics:
    ...
    def concatenateMetricsTable(self):
        vol15Sunday = self.getDailyVolumes('FifteenVolume0', 'Sunday', 'Vol15')
        vol15Monday = self.getDailyVolumes('FifteenVolume1', 'Monday', 'Vol15')
        vol15Tuesday = self.getDailyVolumes('FifteenVolume2', 'Tuesday', 'Vol15')
        vol15Wednesday = self.getDailyVolumes('FifteenVolume3', 'Wednesday', 'Vol15')
        vol15Thursday = self.getDailyVolumes('FifteenVolume4', 'Thursday', 'Vol15')
        vol15Friday = self.getDailyVolumes('FifteenVolume5', 'Friday', 'Vol15')
        vol15Saturday = self.getDailyVolumes('FifteenVolume6', 'Saturday', 'Vol15')
        v = [vol15Sunday,vol15Monday,vol15Tuesday,vol15Wednesday,vol15Thursday,vol15Friday,vol15Saturday]
        vol15 = pd.concat(v)

        truck15Sunday = self.getDailyVolumes('FifteenTruck0', 'Sunday', 'Comm-Pct')
        truck15Monday = self.getDailyVolumes('FifteenTruck1', 'Monday', 'Comm-Pct')
        truck15Tuesday = self.getDailyVolumes('FifteenTruck2', 'Tuesday', 'Comm-Pct')
        truck15Wednesday = self.getDailyVolumes('FifteenTruck3', 'Wednesday', 'Comm-Pct')
        truck15Thursday = self.getDailyVolumes('FifteenTruck4', 'Thursday', 'Comm-Pct')
        truck15Friday = self.getDailyVolumes('FifteenTruck5', 'Friday', 'Comm-Pct')
        truck15Saturday = self.getDailyVolumes('FifteenTruck6', 'Saturday', 'Comm-Pct')
        t = [truck15Sunday,truck15Monday,truck15Tuesday,truck15Wednesday,truck15Thursday,truck15Friday,truck15Saturday]
        truck15 = pd.concat(t)
        self.metrics = self.metrics.merge(vol15, on=['Tmc', 'Time', 'Day'])
        self.metrics = self.metrics.merge(truck15, on=['Tmc', 'Time', 'Day'])

        other_cols = ['Tmc', 'District', 'Segment', 'FFS-14', 'Length-U14', 'vol2019']
